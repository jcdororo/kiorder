...
### 테이블오더 주문 → 주방 실시간 반영
손님이 테이블오더로 주문을 넣으면, 폴링 없이 Supabase Realtime으로 주방 화면 칸반에 즉시 반영됩니다.

![테이블오더 → 주방 실시간 반영](docs/images/GIF-1.gif)

### 키오스크 웨이팅 풀플로우
웨이팅 등록 → QR 대기표 발급 → 손님 대기현황 확인 → 사장님 호출 → 손님 응답까지, 전 과정이 실시간으로 연결됩니다.

![키오스크 웨이팅 풀플로우](docs/images/GIF-2.gif)
